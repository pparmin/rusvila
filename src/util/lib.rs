...
impl Blog {
    ...
    fn write_config(&self) {
        ...
        let config_location = format!(
            "{}/config.json",
            self.location.display().to_string().trim_end_matches("/")
        );
        println!("created config file at {}", config_location);

        let mut config =
            File::create(&config_location).expect("Error while creating project config file");
    ...
    pub fn setup(&mut self, name: &String, path: &Path) {
        ...
        std::fs::write(index_file, INDEX_MD)
            .expect("error while creating index.md in pages directory");
